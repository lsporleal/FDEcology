# -*- coding: utf-8 -*-
"""
Compute yearly NDVI anomaly statistics by phase -- for BOTH the detrended
and non-detrended anomaly, into separate subfolders.

Input:
    NDVI_pentad_5day_with_NDVI_anomaly.csv
    (must contain NDVI_anomaly_detrended and NDVI_anomaly_nondetrended)

Outputs in:
    NDVI_Anomaly_Stats/detrended/
        mean_anomaly.csv
        median_anomaly.csv
        std_anomaly.csv
        min_anomaly.csv
        max_anomaly.csv
        count_anomaly.csv
    NDVI_Anomaly_Stats/nondetrended/
        (same six files)
"""
...
# Config
# =========================================================
INPUT_CSV = r"D:\UNL\FD&Ecosystem\DATA\NEW1\NDVI_pentad_5day_with_NDVI_anomaly.csv"
OUT_DIR = r"D:\UNL\FD&Ecosystem\DATA\NEW1\NDVI_Anomaly_Stats"

PHASE_ORDER = ["normal", "onset", "persistence", "cooldown"]

# Which anomaly columns to process, and the subfolder each goes into
ANOMALY_COLUMNS = {
    "detrended": "NDVI_anomaly_detrended",
    "nondetrended": "NDVI_anomaly_nondetrended",
}
...
def compute_stats_for_column(raw, anomaly_col, sub_out_dir):
    ...
    df = df.rename(columns={"Year": "year", anomaly_col: "NDVI_anomaly"})
    ...
    df = df.dropna(subset=["year", "latitude", "longitude", "NDVI_anomaly"])
    df["year"] = df["year"].astype(int)

    for stat_name, aggfunc in STATS:
        stat_df = make_phase_stat_table(df, "NDVI_anomaly", stat_name, aggfunc)
