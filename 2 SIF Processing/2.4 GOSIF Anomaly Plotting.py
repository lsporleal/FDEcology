...
Produces, for EACH anomaly type (detrended, nondetrended):
  ...
  2) one figure per year (2000-2015)

Plot format (figsize, colors, fonts, gridlines, colorbar, spacing) is
identical to the original single-case version -- only the loop over
ANOMALY_TYPES is new.

Input:
    results/SIF_Anomaly_Stats/<ANOMALY_TYPE>/mean_anomaly.csv
Output:
    results/SIF_Anomaly_Stats/<ANOMALY_TYPE>/plots/mean_SIF_anomaly_by_phase_all_years_US.png
    results/SIF_Anomaly_Stats/<ANOMALY_TYPE>/plots/mean_SIF_anomaly_by_phase_<year>_US.png
"""
...
def plot_phase_map(df_plot, title_suffix, out_name, out_dir):
    ...
    cbar.set_label("Mean SIF anomaly")

    fig.suptitle(
        f"Spatial Pattern of Mean SIF Anomaly by Flash Drought Phase\n{title_suffix}",
        fontsize=15
    )
...
def process_anomaly_type(anomaly_type):
    input_csv = f"results/SIF_Anomaly_Stats/{anomaly_type}/mean_anomaly.csv"
    out_dir = f"results/SIF_Anomaly_Stats/{anomaly_type}/plots"
    ...
    plot_phase_map(
        ...
        out_name="mean_SIF_anomaly_by_phase_all_years_US.png",
        out_dir=out_dir
    )
    ...
    for year in years:
        ...
        plot_phase_map(
            ...
            out_name=f"mean_SIF_anomaly_by_phase_{year}_US.png",
            out_dir=out_dir
        )
